...
--   without replacing the existing database structure.
--
-- IMPORTANT:
--   schema.sql
--   mining_engine.sql
...
--   schema.plus.sql
--
-- Existing mining engine functions are NOT recreated here.
--
-- FEATURES:
--   - Username
--   - Registration notice
...
create unique index if not exists ux_user_social_actions
...
begin
    ...
    v_warning_count := coalesce(v_warning_count, 0) + 1;


    if v_warning_count >= 3 then

        v_suspended_until := now() + interval '30 days';
...
begin
    ...
    -- Missed day resets the corresponding streak.
    ...
    if v_checkin >= 30 and v_boost >= 30 then
    ...
    return jsonb_build_object(
        ...
        'boost_days', v_boost,
        'kyc_face_verification_unlocked', v_face_unlocked,
        'face_verification_unlocked', v_face_unlocked,
        'kyc_face_verified', v_face_verified,
        ...
        'migration_available',
            v_face_verified
            and v_migration_open
            and not v_migration_completed,
        'migration_completed',
            v_migration_completed,
        'coming_soon',
            not v_migration_open,
        'success_message',
...
begin
    ...
    update public.profiles
    set kyc_checkin_streak = v_streak,
...
begin
    ...
    -- A boost streak must come from a real rewarded ad.
    select exists(
        select 1
...
begin
    ...
    select coalesce(kyc_face_verified,false),
           coalesce(migration_completed,false),
           coalesce(fan_balance,0),
           coalesce(afam_balance,0)
    into v_face_verified,
         v_completed,
         v_fan,
         v_afam
    from public.profiles
    ...
    return jsonb_build_object(
        'success', true,
        'migration_open', coalesce(v_open,false),
        'migration_available',
            coalesce(v_open,false)
            and v_face_verified
            and not v_completed,
        'face_verified', v_face_verified,
        'migration_completed', v_completed,
        'fan_balance', v_fan,
        'afam_balance', v_afam,
        'fan_per_afam', coalesce(v_rate,100),
        'message',
            case
        ...
        (
            select jsonb_agg(
                jsonb_build_object(
                    ...
                    'url', t.url,
                    'task_url', t.url,
                    'reward_fan', t.reward_fan,
                    'requires_follow', coalesce(t.requires_follow,false),
                    'requires_like', coalesce(t.requires_like,false),
                    'requires_comment', coalesce(t.requires_comment,false),
                    'requires_share', coalesce(t.requires_share,false),
                    'requires_join', coalesce(t.requires_join,false),
                    'requires_subscribe', coalesce(t.requires_subscribe,false),
                    'is_initial_task', coalesce(t.is_initial_task,false),
                    'task_type', t.task_type,
                    'post_external_id', t.post_external_id,
                    'post_published_at', t.post_published_at,
...
begin
    ...
    if (
        v_requires_follow and not coalesce(v_follow,false)
    )
    or (
        v_requires_like and not coalesce(v_like,false)
    )
    or (
        v_requires_comment and not coalesce(v_comment,false)
    )
    or (
        v_requires_share and not coalesce(v_share,false)
    )
    or (
        v_requires_join and not coalesce(v_join,false)
    )
    or (
        v_requires_subscribe and not coalesce(v_subscribe,false)
    ) then
    ...
    if exists(
        ...
        return jsonb_build_object(
            'success', false,
            'message', 'This social reward has already been claimed.'
        );
    end if;
    ...
    update public.profiles
    set fan_balance = coalesce(fan_balance,0) + v_reward,
    ...
    return jsonb_build_object(
        'success', true,
        'reward_amount', v_reward,
        'message', 'Social reward claimed successfully.'
    );
end;
$$;
...
begin
    ...
    values(
        coalesce(new.title, 'New POWER FAN NETWORK Post'),
        coalesce(new.description, 'Complete Like, Comment and Share to claim your FAN reward.'),
        new.platform,
        new.url,
        new.reward_fan,
        false,
...
-- ============================================================
-- 30. TRUSTED SOCIAL TASK VERIFICATION
-- ============================================================
...
begin
    ...
    if p_action = 'follow' then
        update public.social_task_claims
        set follow_verified = p_verified,
            verified_at = case
        ...
        update public.social_task_claims
        set comment_verified = p_verified,
            verified_at = case
                when p_verified then now()
                else verified_at
            end
        ...
        update public.social_task_claims
        set subscribe_verified = p_verified,
            verified_at = case
                when p_verified then now()
                else verified_at
            end
    ...
    on conflict(user_id, platform, action_type)
    do update set
...
begin
    ...
    return jsonb_build_object(
        ...
        'registration_notice_accepted', v_registration_notice
    );
end;
$$;
...
begin
    ...
    set migration_open = p_open,
        updated_at = now()
    where id = 1;


    update public.profiles
    set migration_available =
        coalesce(kyc_face_verified,false)
        and p_open
...
on public.profiles(kyc_checkin_streak, kyc_boost_streak);
...
alter table public.user_social_actions enable row level security;
alter table public.social_posts enable row level security;
alter table public.social_reward_ledger enable row level security;
...
alter table public.kyc_face_verifications enable row level security;
alter table public.kyc_migration_settings enable row level security;
alter table public.afam_migrations enable row level security;
...
revoke all on public.user_social_actions from anon, authenticated;
revoke all on public.social_reward_ledger from anon, authenticated;
...
revoke all on public.kyc_daily_activity from anon, authenticated;
revoke all on public.kyc_face_verifications from anon, authenticated;
revoke all on public.afam_migrations from anon, authenticated;


-- ============================================================
-- 38. SOCIAL POSTS SELECT
-- ============================================================
...
-- ============================================================
-- 39. MIGRATION SETTINGS SELECT
-- ============================================================

grant select on public.kyc_migration_settings to authenticated;


-- ============================================================
-- 40. CLIENT RPC GRANTS
-- ============================================================

revoke execute on function public.register_device(text,text,text)
from public, anon, authenticated;

grant execute on function public.register_device(text,text,text)
to authenticated;


revoke execute on function public.get_kyc_progress()
from public, anon, authenticated;

grant execute on function public.get_kyc_progress()
to authenticated;


revoke execute on function public.claim_daily_checkin()
from public, anon, authenticated;

grant execute on function public.claim_daily_checkin()
to authenticated;


revoke execute on function public.record_daily_boost()
from public, anon, authenticated;

grant execute on function public.record_daily_boost()
to authenticated;


revoke execute on function public.start_face_verification()
from public, anon, authenticated;

grant execute on function public.start_face_verification()
to authenticated;


revoke execute on function public.complete_face_verification(uuid)
from public, anon, authenticated;

grant execute on function public.complete_face_verification(uuid)
to authenticated;


revoke execute on function public.get_migration_status()
from public, anon, authenticated;
...
revoke execute on function public.migrate_fan_to_afam()
from public, anon, authenticated;

grant execute on function public.migrate_fan_to_afam()
to authenticated;


revoke execute on function public.get_daily_social_tasks()
from public, anon, authenticated;

grant execute on function public.get_daily_social_tasks()
to authenticated;


revoke execute on function public.start_social_task(uuid)
from public, anon, authenticated;

grant execute on function public.start_social_task(uuid)
to authenticated;


revoke execute on function public.claim_daily_social_reward(uuid)
from public, anon, authenticated;

grant execute on function public.claim_daily_social_reward(uuid)
to authenticated;


revoke execute on function public.are_initial_social_tasks_complete(uuid)
from public, anon, authenticated;

grant execute on function public.are_initial_social_tasks_complete(uuid)
to authenticated;


revoke execute on function public.accept_registration_notice()
from public, anon, authenticated;

grant execute on function public.accept_registration_notice()
to authenticated;


revoke execute on function public.get_account_security_status()
from public, anon, authenticated;

grant execute on function public.get_account_security_status()
to authenticated;


-- ============================================================
-- 41. TRUSTED BACKEND RPC SECURITY
-- ============================================================

revoke execute on function public.confirm_face_verification(uuid,text)
from public, anon, authenticated;


revoke execute on function public.verify_social_task_action(uuid,uuid,text,boolean)
from public, anon, authenticated;


revoke execute on function public.set_migration_status(boolean)
from public, anon, authenticated;


-- ============================================================
-- 42. PROTECT SENSITIVE PROFILE UPDATES
-- ============================================================
...
alter table public.profiles
    alter column kyc_checkin_streak set default 0;

alter table public.profiles
    alter column kyc_boost_streak set default 0;

alter table public.profiles
    alter column kyc_face_verification_unlocked set default false;

alter table public.profiles
    alter column kyc_face_verified set default false;

alter table public.profiles
    alter column migration_available set default false;

alter table public.profiles
    alter column migration_completed set default false;

alter table public.profiles
    alter column registration_notice_accepted set default false;

alter table public.profiles
    alter column device_warning_count set default 0;


-- ============================================================
-- 44. FINAL MIGRATION DEFAULT
-- ============================================================

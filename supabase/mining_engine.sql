...
--   Ad rewards are NOT retroactive.
--   Referral rewards are NOT retroactive.
--   Maximum rewarded ads = 7 per mining session.
-- ============================================================
    ...
    select coalesce(
        sum(
            (
                extract(
                    epoch from (
                        p_ends_at - greatest(ar.watched_at, p_started_at)
                    )
                ) / 3600.0
            )
            *

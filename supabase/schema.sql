...
alter table public.profiles add column if not exists fan_balance numeric(24,8) default 0;
alter table public.profiles add column if not exists afam_balance numeric(24,8) default 0;
alter table public.profiles add column if not exists mining_rate numeric(12,4) default 0.20;
alter table public.profiles add column if not exists active_referrals integer default 0;
alter table public.profiles add column if not exists daily_ads_watched integer default 0;
alter table public.profiles add column if not exists ad_boost numeric(12,4) default 0;
alter table public.profiles add column if not exists mining_active boolean default false;
alter table public.profiles add column if not exists mining_started_at timestamptz;
alter table public.profiles add column if not exists mining_ends_at timestamptz;
alter table public.profiles add column if not exists consecutive_check_ins integer default 0;
alter table public.profiles add column if not exists kyc1_eligible boolean default false;
alter table public.profiles add column if not exists kyc1_verified boolean default false;
alter table public.profiles add column if not exists kyc2_eligible boolean default false;
alter table public.profiles add column if not exists kyc2_verified boolean default false;
alter table public.profiles add column if not exists kyc3_verified boolean default false;
alter table public.profiles add column if not exists last_social_claim_date date;
...
alter table public.mining_sessions add column if not exists user_id uuid;
alter table public.mining_sessions add column if not exists started_at timestamptz default now();
alter table public.mining_sessions add column if not exists ends_at timestamptz;
alter table public.mining_sessions add column if not exists mining_rate numeric(12,4) default 0.20;
alter table public.mining_sessions add column if not exists claimed boolean default false;
alter table public.mining_sessions add column if not exists claimed_at timestamptz;
alter table public.mining_sessions add column if not exists reward_amount numeric(24,8) default 0;
alter table public.mining_sessions add column if not exists created_at timestamptz default now();
...
alter table public.ad_rewards add column if not exists user_id uuid;
alter table public.ad_rewards add column if not exists session_id uuid;
alter table public.ad_rewards add column if not exists ad_number integer;
alter table public.ad_rewards add column if not exists watched_at timestamptz default now();
alter table public.ad_rewards add column if not exists reward_amount numeric(12,4) default 0.10;
alter table public.ad_rewards add column if not exists created_at timestamptz default now();
...
alter table public.referrals add column if not exists status text default 'active';
alter table public.referrals add column if not exists new_user_reward numeric(24,8) default 20;
alter table public.referrals add column if not exists referrer_reward numeric(24,8) default 5;
alter table public.referrals add column if not exists mining_rate_bonus numeric(12,4) default 0.02;
...
alter table public.wallet_transactions add column if not exists coin text;
alter table public.wallet_transactions add column if not exists transaction_type text;
alter table public.wallet_transactions add column if not exists amount numeric(24,8);
...
alter table public.social_tasks add column if not exists reward_fan numeric(24,8) default 0;
alter table public.social_tasks add column if not exists requires_follow boolean default true;
alter table public.social_tasks add column if not exists requires_like boolean default true;
alter table public.social_tasks add column if not exists requires_comment boolean default true;
alter table public.social_tasks add column if not exists requires_share boolean default true;
alter table public.social_tasks add column if not exists is_active boolean default true;
alter table public.social_tasks add column if not exists post_external_id text;
alter table public.social_tasks add column if not exists post_published_at timestamptz;
alter table public.social_tasks add column if not exists starts_at timestamptz;
alter table public.social_tasks add column if not exists ends_at timestamptz;
alter table public.social_tasks add column if not exists created_at timestamptz default now();
alter table public.social_tasks add column if not exists updated_at timestamptz default now();
...
alter table public.social_task_claims add column if not exists follow_verified boolean default false;
alter table public.social_task_claims add column if not exists like_verified boolean default false;
alter table public.social_task_claims add column if not exists comment_verified boolean default false;
alter table public.social_task_claims add column if not exists share_verified boolean default false;
alter table public.social_task_claims add column if not exists reward_amount numeric(24,8) default 0;
alter table public.social_task_claims add column if not exists started_at timestamptz;
alter table public.social_task_claims add column if not exists verified_at timestamptz;
alter table public.social_task_claims add column if not exists claimed_at timestamptz;
alter table public.social_task_claims add column if not exists created_at timestamptz default now();
...
select 'minimum_supported_version', '1.0.0', now()
where not exists (
    select 1
...
on public.profiles(mining_active, mining_started_at, mining_ends_at);

create index if not exists idx_mining_sessions_user
on public.mining_sessions(user_id, started_at desc);

create index if not exists idx_mining_sessions_active
on public.mining_sessions(user_id, claimed, ends_at);

create index if not exists idx_ad_rewards_user
on public.ad_rewards(user_id, watched_at desc);

create index if not exists idx_ad_rewards_session
on public.ad_rewards(session_id, watched_at);
...
on public.kyc_verifications(user_id, phase);

create index if not exists idx_social_tasks_active
on public.social_tasks(is_active, starts_at, ends_at);

create index if not exists idx_social_claims_user
on public.social_task_claims(user_id, task_date desc);

create index if not exists idx_social_follows_user
on public.user_social_follows(user_id, platform);
...
create unique index if not exists ux_ad_rewards_session_ad_number
on public.ad_rewards(session_id, ad_number)
...
create unique index if not exists ux_profiles_referral_code
...
create or replace function public.generate_referral_code(input_name text)
returns text
language plpgsql
security definer
...
--
-- IMPORTANT:
-- The registration screen sends referral_code in
...
begin
    ...
    -- Prevent a second referral relationship.
    if exists (
        select 1
        from public.profiles
        where id = new.id
    ...
    -- Create referral record first. The unique partial index on referred_id
    -- cannot be targeted by ON CONFLICT (referred_id), so handle a race
    -- with a local unique_violation block instead.
    begin
    ...
    -- Give the new user reward only after the referral row exists.
    ...
    -- Give inviter reward. active_referrals is intentionally NOT incremented
    -- here because active referrals are calculated from live mining status.
    update public.profiles
    set

...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _updateTime() {
    ...
    if (_startedAt != null) {
      ...
      final elapsed = now.difference(calculatedStart);
  ...
  Future<void> _claimMining() async {
    ...
    try {
      ...
      final claimable = latest['claimable'] ?? false;
  ...
  DailySocialTask _findTask(
    String taskId, {
    required DailySocialTask fallback,
  }) {
              ...
              children: [
                Text(
                  _t(
                    ...
                    'Complete the social actions, then verify and claim your reward.',
                  ),
                  style: const TextStyle(
                    fontSize: 13,
                    color: Colors.grey,
  ...
  DailySocialTask? _findTaskOrNull(
    String taskId,
  ) {
    for (final task in _tasks) {
      if (task.id == taskId) {

...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _loadMining() async {
    ...
    if (finalStarted == null && finalEnds != null) {
      finalStarted = finalEnds.subtract(
        miningDuration,
      );
    }

    if (finalEnds == null && finalStarted != null) {
      finalEnds = finalStarted.add(
        miningDuration,
      );
    }
    ...
    final activeFlag = data['mining_active'];
    ...
    if (finalCanClaim &&
        ...
        rate > 0) {
      liveReward =
          (miningDuration.inSeconds / 3600.0) * rate;
    }
  ...
  Widget _buildMiningCard() {
    ...
    final buttonText = _claimAdWaiting
        ? 'WATCHING AD...'
    ...
    final buttonIcon = _claimAdWaiting
    ...
    final description =
        isReadyToClaim
            ? 'Your mining session is complete and ready to claim after 24-hour'
            : _isMining

...
//
// IMPORTANT:
// Opening the social URL does NOT automatically claim reward.
...
try {
  ...
  await _showSocialTaskDialog(updated);
} catch (e) {
  _message(_error(e));
} finally {
  if (mounted) {
    ...
    return AlertDialog(
      ...
      content: Column(
        ...
        children: [
          Text(
            _t(
              'complete_task_then_verify',
              'Complete the social task, then verify and claim your reward.',
            ),
            style: const TextStyle(
              fontSize: 13,
              color: Colors.grey,
...
try {
  // Always refresh the task from Supabase
  // before attempting the claim.
  await _loadTasks();
  ...
  // Server-side verification status must
  // say the task can be claimed.
  if (!task.canClaim) {
    await _showSocialTaskDialog(
      task,
    );
    ...
    _message(
      _t(
        'task_not_verified',
        'Task has not been verified yet. Complete the required action and try again.',
      ),
    );
  ...
  if (claim['success'] == true) {
    ...
    _message(
      message != null &&
          ...
          : _t(
              'task_not_verified',
              'Task has not been verified yet.',
            ),
    );
  }
} catch (e) {
  ...
  child: RefreshIndicator(
    ...
    child: ListView(
      ...
      children: [
        ...
        // Boost Ads remains in the Home screen.
        // Rewarded Ads integration will be completed
        // later after the ad account and IDs are ready.
        _boostCard(),
...
/*
 * Rewarded Ads integration will be connected
 * to the final ad provider/account later.
 *
 * When the account is ready, we will add:
 * - App ID
 * - Rewarded Ad Unit ID
 * - Account/provider configuration
 * - Reward verification
 * - +0.1 FAN/H per completed ad
 * - maximum 7 ads per mining session
 *
 * For now this callback only refreshes data.
 */

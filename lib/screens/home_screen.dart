...
class _HomeScreenState extends State<HomeScreen> {
  ...
  /// Reward earned by the current unclaimed
  /// mining session according to the server.
  ...
  Future<void> _loadMining() async {
    ...
    /*
     * When the server gives us remaining_seconds,
     * use it immediately.
     *
     * This is important when the phone was OFFLINE.
     * When the user opens the app again, Supabase
     * has already calculated the correct remaining
     * time from the real session timestamps.
     */
    ...
    setState(() {
      ...
      /*
       * This is the server-calculated reward
       * for the current session.
       *
       * We DO NOT calculate the reward from
       * the time the app was open.
       */
      _sessionReward =
          serverReward;
    ...
    /*
     * Start a local one-second display timer.
     *
     * The timer is ONLY for updating the UI.
     * The actual session remains controlled by
     * started_at / ends_at and Supabase.
     */
    if (_isMining) {
      _startTimer();
    }
  ...
  void _startTimer() {
    ...
    _timer = Timer.periodic(
      ...
      (_) {
        ...
        /*
         * The server reward loaded at the last
         * refresh is the starting point.
         *
         * While the session is active, advance
         * the displayed reward by the current
         * mining rate.
         *
         * Base:
         *
         * 0.20 FAN / 3600 seconds
         * = 0.000055555...
         *
         * Displayed as:
         *
         * 0.00005556 FAN / second
         */
  ...
  Future<void> _watchAd() async {
    ...
    try {
      final shown =
        ...
        onRewarded: () {
          /*
           * Reload from Supabase after the
           * rewarded-ad callback.
           *
           * This makes the new ad boost start
           * at the correct server timestamp.
           */
          unawaited(
            _loadMining(),
          );
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Follow and get ${_formatFan(reward)} FAN reward',
                  style:
                      const TextStyle(
                    fontSize: 13,
                    color:

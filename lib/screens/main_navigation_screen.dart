...
import '../boost_ads_card.dart'; // <-- WANNAN SHINE GYAR. BA components BA
...
class _MainNavigationScreenState extends State<MainNavigationScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if(mounted) setState(() {
        ...
        _isMining = mining['is_active']?? false; 
        _miningRate = rate; 
        _socialTasks = tasks; 
        _loading = false;
      });
  ...
  Future<void> _openLinkAndClaim(DailySocialTask task) async {
    ...
    try { 
      await _socialTaskService.verifyAndClaim(taskId: task.id); 
      _loadData(); 
    } catch (e) {}
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(16), children: [
        BoostAdsCard(onBoostUpdated: _loadData, miningRate: _miningRate, isMining: _isMining), // <-- YANZU ZAI GANE SHI
        const SizedBox(height: 20), 
        const Text('Daily Social Tasks', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        ..._socialTasks.map((task) => ListTile(
          title: Text(task.title), 
          subtitle: Text(task.description),
          trailing: task.claimed ? const Icon(Icons.check, color: Colors.green) : ElevatedButton(
            child: const Text('Claim'), 
            onPressed: () => _openLinkAndClaim(task)
          ),
        )),
      ]),
    );

...
class BoostAdsCard extends StatefulWidget {
  ...
  final bool isMining;
  const BoostAdsCard({super.key, required this.onBoostUpdated, required this.miningRate, required this.isMining});
  @override
...
class _BoostAdsCardState extends State<BoostAdsCard> {
  ...
  int _adsWatched = 0; bool _loading = false;
  @override
  ...
  Future<void> _watchAd() async {
    ...
    } catch (e) { _showMessage(e.toString().replaceAll('Exception: ', '')); }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      padding: const EdgeInsets.all(16), margin: const EdgeInsets.symmetric(vertical: 8),
      ...
      child: Column(children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          ...
          Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: isFull ? Colors.red.shade100 : Colors.green.shade100, borderRadius: BorderRadius.circular(20)), child: Text('$_adsWatched / 7', style: TextStyle(color: isFull ? Colors.red : Colors.green, fontWeight: FontWeight.bold)))
        ]),
        ...
        SizedBox(width: double.infinity, child: ElevatedButton(
          ...
          style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF3B159B), disabledBackgroundColor: Colors.grey.shade300, padding: const EdgeInsets.symmetric(vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
          child: _loading ? const CircularProgressIndicator(color: Colors.white) : Text(isFull ? 'An Kammala 7/7' : 'Watch Ad +0.10 FAN/H'),
        ))
      ]),
    );
  }
  ...
  DateTime? _parseDateTime(dynamic value) { if (value == null) return null; if (value is DateTime) return value.toLocal(); return DateTime.tryParse(value.toString())?.toLocal(); }
}
